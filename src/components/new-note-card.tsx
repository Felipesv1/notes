...
export function NewNoteCard({ onNoteCreated }: NewNoteCardProps) {
  ...
  function handleStartRecording() {
    ...
    speechRecognition.onresult = (ev) => {
      const transcription = Array.from(ev.results).reduce((text, result) => {
    ...
    <Dialog.Root>
      <Dialog.Trigger className="rounded-md flex flex-col bg-slate-700 p-5 gap-3 text-left outline-none hover:ring-2 hover:ring-slate-600 focus-visible:ring-2 focus-visible:ring-lime-400">
        ...
        <p className="text-sm leading-6 text-slate-400">
          Grave uma nota em áudio que sera convertida para texto
          automaticamente.
        </p>
      </Dialog.Trigger>
      <Dialog.Portal>
          ...
          <form onSubmit={handleSaveNote} className="flex-1 flex flex-col">
            ...
            {isRecording ? (
              <button
                ...
                className="w-full flex items-center gap-2 justify-center bg-slate-900 py-4 text-center text-sm text-slate-300 outline-none font-medium hover:text-slate-100"
              >
